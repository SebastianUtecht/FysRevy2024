...
function handleDomLoaded() {
  ...
  for (let i = 0; i < slideElNodeList.length; i++) {
    slideEls.push(slideElNodeList[i]);
  }

  for (let i = 0, slide; slide = slideEls[i]; ++i) {
...
function updateSlides( seekMedia = false ) {
  ...
  stopAllPlayback( true /* onlyOtherSlides */ );
...
function makeWindows() {
  ...
  ann.appendChild(list);
   ...
   $(document).one( "load", () => console.log( "PING" ) )
    setTimeout( () => {
	console.log( document.body.offsetHeight ) // force reflow
      requestAnimationFrame( () => {
         let singleScaleSheet = document.createElement('style');
         let scaleFactor = ( 200 / $(".slides article").first().height() )
         document.head.appendChild(singleScaleSheet);
         singleScaleSheet.sheet.insertRule('.slides article { transform-origin: top left; margin-right: ' + (($(".slides article").first().width() ) * ( scaleFactor - 1 ) ) + 'px; }')
         
         singleScaleSheet.sheet.insertRule('.slides article { transform: scale(' + scaleFactor + '); }')
          setTimeout( bigSlideScale, 100 )
      })
    }, 100)/* så er browseren nok i gang med at layout'e... *//* update: neeeej! */
...
function listMediaIn(node, overDiscriminator) {
   ...
   /* ...den her funktion bruger ikke sine argumenter...? */
...
function controlPlayback( fromStart = false ) {
  ...
  for (var i = 0; i < media.length; i++) {
     if (!media[i].matches('.last-build > .rununder')
        && !media[i].matches("article > .rununder")
...
function stopAllPlayback( onlyOtherSlides = false ) {
  console.log( onlyOtherSlides ? "stopOutside" : "stopall" );
  var audio = document.getElementsByTagName('audio');
  var video = document.getElementsByTagName('video');
  var media = Array.prototype.slice.call(audio)
      .concat(Array.prototype.slice.call(video));
  var insideMedia = listMediaIn(slideEls[curSlide], ".mimicking");

   for(var i = 0; i < media.length; i++) {
     if ( overSide ) media[i].muted = true
     if ( onlyOtherSlides && insideMedia.includes( media[i] ) ) continue;
     media[i].pause();
  }
};
