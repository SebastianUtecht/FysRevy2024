...
@font-face{
    font-family: 'CMU Upright Italic';
    src: local('cmunui'),
    local('CMU Serif Upright Italic'),
    url(/node_modules/@fysikrevy/woff/cmunui.woff) format('woff');
}
...
.lyr > p.left, .lyr > p.right {
    width: 80%;
    height: 2.6em;
    margin-bottom: calc( -2.6em - 400px );
}

.lyr > p.left:not(.to-build), .lyr > p.right:not(.to-build) {
    opacity: 1;
}
...
body.right section.slides article[for="span"]{
    transform: scale(.5);/* scale(2) translate( -350px, -20px ) skewX(7.5deg ); */
}
