...
var h2Out = getNewOutGroup();
h2Out.setTitle("HEAD");
 partOut.addNext(async function h2() {
    function finish(){
       h2Out.log( response.status + " " + response.statusText )
       h2Out.next()
    }
    getTestDiv.innerHTML += "<h4>HEAD request</h4>"
    getTestDiv.appendChild( h2Out );
    h2Out.log( "Sending HEAD request" )
    h2Out.setStatus( "loading" )
    const response = await fetch( urlBase + "test.html", { method: "HEAD" } )
    if ( !response.ok ){
       h2Out.log( "Fetch failed:" )
       h2Out.setStatus( "error" )
       return finish()
    }
    let sawAHeader = false
    h2Out.log( "Enumerating headers:" )
    for ( e of response.headers.entries() ){
       h2Out.log( e )
       sawAHeader = true
    }
    if ( !sawAHeader ){
       h2Out.log( "There were no headers. There should be." )
       h2Out.setStatus( "error" )
       return finish()
    }
    h2Out.log( "--End of headers" )
    if ( (await response.text()).length == 0 ){
       h2Out.log( "No body, OK." )
    } else {
       h2Out.log( "Had body. That's wrong" )
       h2Out.setStatus( "error" )
       return finish()
    }
    h2Out.setStatus( "loaded" )
    return finish()
 })
...
function issueCommand(cmd, out) {
    ...
    if (cmd.localeCompare("goto") == 0 || cmd.localeCompare("nej") == 0) {
        xhr.setRequestHeader('slide',gotoContent[0]);
        xhr.setRequestHeader('item',gotoContent[1]);
        xhr.setRequestHeader('x-referer', urlLoc + "test/test-struc.html" )
    }
...
var nejOut = getNewOutGroup();
nejOut.setTitle("'nej'");
conOut.addNext(function nejSet() {
    streamSec.innerHTML += "<h4>Source rewrite on 'nej'</h4>"
    streamSec.appendChild( nejOut );
    var timeout = 5000;
    var nejTimer = setTimeout( function nejTimeout() {
       nejOut.log( "Connection timeout after " + timeout + "ms");
       nejOut.setStatus( "error" );
       nejOut.next();
       nej.close();
    }, timeout );
    nejOut.log( "Connecting to " + urlLoc + "stream" );
    conOut.setStatus( "loading" );
    var nej = new EventSource( urlLoc + "stream" );
    async function checkForNej( e ){
       nejOut.log( "Give file write a chance to complete..." )
       await new Promise( resolve => setTimeout( resolve, 100 ) )
       nejOut.log("Retrieving test-struc.html")
       try {
          var response = await fetch( urlLoc + "test/test-struc.html" )
       } catch (er) {
          nejOut.log( er )
          nejOut.log( "Failed to retrieve test-struc.html: network error. Test inconclusive.")
          nejOut.setStatus( "warning" )
          throw er
       }
       if ( !response.ok ){
          nejOut.log( "Failed to retrieve test-struc.html: response was " + response.statusText + ". Test inconclusive." )
          nejOut.setStatus( "warning" )
          throw null
       }
       nejOut.log( "Recieved test-struc.html, testing for presence of 'nej'-class" )
       let match = /<[^>]*id="here"[^>]*>/.exec( await response.text() )
       if ( match === null ){
          nejOut.log( html )
          nejOut.log( "Something is wrong with test-struc.html. Test inconclusive.")
          nejOut.setStatus( "warning" )
          throw null
       }
       return match[0].includes( "nej" )
    }
    nej.addEventListener("connections",  e => { 
       nejOut.log("Recieved: (" + e.lastEventId + ") "
                + e.type + ": " + e.data);
    })
    nej.addEventListener("connections", e => {
       nej.addEventListener("nej", async function nejTest( e ) {
          try {
             clearTimeout(nejTimer);
             nejOut.log("Recieved: (" + e.lastEventId + ") "
                      + e.type + ": " + e.data);
             const foundNej = await checkForNej()
             if ( foundNej ){
                nejOut.log( "'nej' located." )
             } else {
                nejOut.log( "'nej' not located. Unexpected, but maybe okay." )
             }
             nejOut.log( "Trying again for toggle." )

             let e2 = await new Promise( (resolve, reject) => {
                let nejTimer = setTimeout( function nejTimeout() {
                   nejOut.log( "Connection timeout after " + timeout + "ms" )
                   nejOut.setStatus( "error" )
                   reject( "timeout" )
                }, timeout)
                nej.addEventListener( "nej", e => {
                   clearTimeout( nejTimer )
                   resolve( e )
                }, {once:true} )
                issueCommand( "nej", nejOut )
             })

             const foundNej2 = await checkForNej()
             if ( foundNej2 ) {
                nejOut.log( "'nej' found after second command." )
             } else {
                nejOut.log( "No 'nej' found after second command." )
             }
             if ( foundNej == foundNej2 ){
                nejOut.log( "Failed" )
                nejOut.setStatus( "error" )
                throw null
             }
             nejOut.log( "OK" )
             nejOut.setStatus( "loaded" )
             return
          } catch (er) {
             nejOut.log( er )
          } finally {
...
var specialCmdList = ["d"];
function addAllCmdObservers(es, out) {
    infoList.concat(cmdList).concat(specialCmdList).forEach( e => {
...
nejOut.addNext(function sseAllTest() {
    ...
    for (var i = 0; i < numListeners; i++) {
        ...
        function waitForNextCmd( e ) {
            ...
            } else if (step < cmdList.length + specialCmdList.length) {
                waitFor( specialCmdList[step - cmdList.length]);
                anOut.listener.addEventListener(
                    specialCmdList[step - cmdList.length], waitForNextCmd,
                    {once: true});
               if (e.target == testOuts[numListeners - 1].listener) {
                  if ( specialCmdList[step - cmdList.length] == "d" ){
                   var xhr = new XMLHttpRequest();
	                xhr.open("post", urlLoc + "cmd");
	                xhr.setRequestHeader('Content-Type','text/plain');
	                xhr.setRequestHeader('sl-cmd',
                                        specialCmdList[step - cmdList.length]);
                   sseAllOut.log("Issuing command \""
                     ...
                     issueCommand( specialCmlList[step - cmdList.lenght] );
                  }
                }
            } else {
                if (anOut.cTimeout)
